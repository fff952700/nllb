...
class CPUTranslator(BaseTranslator):
    """基于 CTranslate2（CPU int8）的翻译器，懒加载模型防止多进程 fork 死锁"""
    ...
    def _load(self):
        ...
        if self._translator is None:
            print("CPUTranslator: Initializing tokenizer...", flush=True)
            self._tokenizer = AutoTokenizer.from_pretrained(self._cfg.name)
            print("CPUTranslator: Initializing CTranslate2 (CPU, int8)...", flush=True)
            self._translator = ctranslate2.Translator(
                self._cfg.ct2_path,
                device="cpu",
                compute_type="int8",
            )
            print("CPUTranslator: Ready.", flush=True)
